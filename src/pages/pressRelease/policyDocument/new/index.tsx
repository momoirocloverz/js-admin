...
const PolicyNewDocumentPage = (props: any) => {
  ...
  const [isInclusivePolicy, setIsInclusivePolicy] = useState(false);
  ...
  // const [dateCheck, setDateCheck] = useState(moment('2015-06-06', dateFormat));
  ...
  const fetchInclusivePolicyItemInfo = () => {
    let outer = categoryArray.find((ele) => {
      return ele.category_name == '惠农补贴';
    });
    if (outer) {
      let sec = outer.get_parent_do.filter((sub) => {
        return (
          sub.category_name == '秸秆综合利用补贴' ||
          sub.category_name == '有机肥使用补贴'
        );
      });
      let after = sec.map((ele) => {
        return ele.id;
      });
  ...
  const addRequest = (bridge: Object) => {
    // return console.log('来请求了', bridge);
    Apis.policyDocumentAdd(bridge)
      .then((res: any) => {
        if (res && res.code === 0) {
    ...
    dynamicBottom = (
      <>
        {/* <Form.Item
          ...
          label="是否为竞争性财政支农项目"
        >
          <Radio.Group onChange={onRadioChange} value={is_contend}>
  ...
  const okHandle = async () => {
    ...
    let trackParent = categoryArray.find((ele) => {
      return ele.category_name == '惠农补贴';
    });
...
export default connect()(PolicyNewDocumentPage);
